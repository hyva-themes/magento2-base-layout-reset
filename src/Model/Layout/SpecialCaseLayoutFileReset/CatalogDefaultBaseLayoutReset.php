...
class CatalogDefaultBaseLayoutReset implements SpecialCaseBaseLayoutResetInterface
{
    ...
    {
        return $module === 'Magento_Catalog' && in_array($filename, ['default.xml', 'empty.xml'], true);
    }

    public function resetLayout(\SimpleXMLElement $layoutXml): void
    {
        $this->genericBaseLayoutFileReset->resetLayout($layoutXml, /* Skip xpaths */ ['//block']);
        // Keep price renderer block
        $this->mutateXml->removeXpath($layoutXml, '//block', function (\DOMElement $dom): bool {
            return $dom->getAttribute('name') !== 'product.price.render.default';
        });
    }
}
